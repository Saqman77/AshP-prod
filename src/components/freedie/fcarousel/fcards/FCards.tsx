...
const ShoutoutPopup: React.FC<{ message: string }> = ({ message }) => {
  ...
  useEffect(() => {
    if (message) {
      ...
      const fadeTimer = setTimeout(() => setFadeOut(true), 4000); // Start fade-out
      const hideTimer = setTimeout(() => setVisible(false), 4500); // Hide after fade-out
...
const FCards: React.FC<FCardsProps> = ({ id, img, desc, role, mail, portfolio, fb, insta, x, link, spotify, tree, yt, sc, bandC, git, behance, dribble, shout, cv }) => {
    ...
    // <div>
      ...
      <div className='f-card' id={id} ref={carouselRef}>
        ...
        <div className="card-top">
          <div className="left-img" style={{ backgroundImage: `url(${img})`, backgroundPosition: 'center', backgroundRepeat: 'no-repeat', backgroundSize:'100% 100%' }}>
